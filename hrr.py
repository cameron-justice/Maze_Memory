...
class LTM:
    ...
    def __init__(self,N=1024,M=1,normalized=True):
        # self.store = shelve.open(self.tmpdir + "hrr_ltm_" + str(os.getpid()) + "_" + str(id(self)))
        self.store = dict()
        self.N = N
        self.M = M
        self.normalized = normalized
        self.store[self.identity_string] = np.array([hrri(self.N) for x in range(self.M)])
    ...
    def lookup(self,q):
        ...
        if q not in self.store:
            self.store[q] = np.array([hrr(self.N,self.normalized) for x in range(self.M)])
        return self.store[q]
            ...
            if key not in self.store:
                ...
                for L in range(1,len(substr)):
                    for combination in itertools.combinations(substr,L+1):
                        ...
                        # print(key)
                        if key not in self.store:
                            subrep = self.store[self.identity_string]
                            for i in range(len(combination)):
                                combrep = self.lookup(combination[i])
                                subrep = np.array([convolve(subrep[x],combrep[x]) for x in range(self.M)])
    ...
    def decode(self,q):
        ...
        for key in self.store.keys():
            if numpy.mean(numpy.dot(q,self.store[key].transpose())) > best:
                best = numpy.mean(numpy.dot(q,self.store[key].transpose()))
                match = key
        return match
